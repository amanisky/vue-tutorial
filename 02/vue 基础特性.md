...
#### 与传统事件绑定的区别
+ 最开始就是将 HTML 与 JS 隔离开编写，但其实事件处理方法和表达式都严格绑定在当前视图的 ViewModel 上，并不会导致维护困难；这么些的好处在于：
+ 无需手动管理事件
    + ViewModel 被销毁时，所有的事件处理程序都会自动被删除，让我们从获取 DOM 绑定事件然后在特定情况下再解绑这样的事情中解脱出来
+ 解耦
    + ViewModel 代码是纯粹的逻辑代码，与 DOM 无关，有利于写自动化测试用例